...
public class DownloadParser {
  ...
  public static String downLoad(ZxxkPaper paper) throws Exception{
    try{
      ...
      if(StringUtils.isBlank(paper.getDownloadUrl())) {
        HttpResponse res = client.request(request, 10000);
        ...
        request.setUrl(paper.getDownloadUrl());
      }

      HttpResponse res1 =  client.request(request, 600000);
      String path = writer2File(res1.getBody(),paper.getName()+"."+paper.getExt(),paper.getType1());
      paper.setLocalPath(path);
      return path;
    }catch(Exception e){
      log.error("dwonload error ",e);
    }
    return null;
  }
