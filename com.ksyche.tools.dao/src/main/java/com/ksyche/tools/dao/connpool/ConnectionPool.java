...
public class ConnectionPool {
	...
	private Stack<Connection> AvailableConn = new Stack<Connection>(); // Connections
																		// stack
	private DBConfig _Config; // Pool's configuration.
	private List<Connection> Pool = new ArrayList<Connection>(); // Here store
																	// all
																	// connections.
	...
	/**
	 * 2011-05-24 获得一个只需要读的数据库连接,处理主库压力大的情况下，降低主库压力 by renjun
	 * 
		...
		if (conn != null) {
			try {
				if (_Config.getAutoShrink() // is allow auto shrink pool
						&& state.GetNoWorkCount(_Config.getIdleTimeout() * 1000) > 0// check
																					// no
																					// work
																					// connections
																					// count
	...
	private void LoadDrivers() {
		try {
			Driver driver = (Driver) Class.forName(_Config.getDriversClass())
					.newInstance();
	...
	private void RegisterExcetEven() {
		Runtime.getRuntime().addShutdownHook(new Thread() {
			public void run() {
				System.out
						.println("application exiting,begin remove all connections.");
